...
import {
 getVacations, getVacationStatuses, updateVacationStatus
} from "./../../../api/index";
...
export default function VacationTable({ status, editable }: Props) {
  ...
  const handleStatusChange = async (vacationId: number, newStatusId: number) => {
    try {
                ...
                <TableCell>
                  {editable ? (
                    <FormControl size="small" sx={{ minWidth: 150 }}>
                      <Select
                        value={statuses.some(s => s.id === vacation.status.id) ? vacation.status.id : ""}
                        onChange={(e) =>
                          handleStatusChange(vacation.id, Number(e.target.value))
                        }
                      >
                        {statuses.map((status) => (
